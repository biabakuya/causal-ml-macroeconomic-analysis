    ...
    for package_info in packages:
        ...
        try:
            ...
            if module_name in ['pygraphviz', 'graphviz']:
                print(f"     {display_name:<30} (optionnel, non installé)")
...
try:
    ...
    if result.returncode == 0:
        ...
        print(f"     Graphviz (système)          Non détecté (optionnel)")
...
if warnings_packages:
    print(f"  Avertissements : {len(warnings_packages)}")
...
if success_rate >= 90 and not failed_packages:
    ...
    print(" INSTALLATION PARTIELLE - Quelques packages manquent")
